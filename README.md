...
| error: no match for ‘operator>=’ (operand types are ‘FILE’ {aka ‘_IO_FILE’} and ‘char’)
   26 |         if ((stdin[i] >= 'a' && stdin[i] <= 'z') || (stdin[i] >= 'A' && stdin[i] <= 'Z')) | You cannot use stdin here, use the variable text instead. |
